	...
	view_as_skill(NULL), filter(NULL)
{
	...
	m_cardSpecialRegion = QRectF(specialCenter.x() - G_COMMON_LAYOUT.m_cardNormalWidth * 1.5,
								  specialCenter.y() - G_COMMON_LAYOUT.m_cardNormalHeight,
								  G_COMMON_LAYOUT.m_cardNormalWidth * 3,
...
void Dashboard::updateAvatar(){
	...
	avatar->setPixmap(G_ROOM_SKIN.getGeneralPixmap(general->objectName(), QSanRoomSkin::S_GENERAL_ICON_SIZE_LARGE));
...
void Dashboard::updateSmallAvatar(){
	...
	if(general2){
		...
		small_avatar->setPixmap(G_ROOM_SKIN.getGeneralPixmap(general2->objectName(), QSanRoomSkin::S_GENERAL_ICON_SIZE_LARGE));
	}
...
void Dashboard::_installDelayedTrick(CardItem *card){
	...
	item->setPixmap(G_ROOM_SKIN.getCardSuitPixmap(trick->getSuit()));
	QString tooltip;
...
void Dashboard::setWidth(int width){
	...
	m_cardTakeOffRegion.setY(middle->pos().y() - G_COMMON_LAYOUT.m_cardNormalHeight * 1.5);
	m_cardTakeOffRegion.setHeight(G_COMMON_LAYOUT.m_cardNormalHeight);
}
...
void Dashboard::drawEquip(QPainter *painter, const CardItem *equip, int order){
	...
	painter->drawPixmap(suit_rect, G_ROOM_SKIN.getCardSuitPixmap(equip->getCard()->getSuit()));


	// draw the number of equip
	painter->drawPixmap(width - 14, y + 3, G_ROOM_SKIN.getCardNumberPixmap(equip->getCard()->getNumber(), equip->getCard()->isBlack()));
...
void Dashboard::_adjustCards(const QList<CardItem *> &list, int y){    
	...
	int start_x = left->boundingRect().width() + G_COMMON_LAYOUT.m_cardNormalWidth / 2;
