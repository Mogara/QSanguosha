	...
	main_window(main_window),game_started(false)
{
	...
	_m_roomLayout = &(_m_roomSkin->getRoomLayout());
...
void RoomScene::handleEventEffect(const Json::Value &arg)
{
	...
	if (eventType == S_GAME_EVENT_PLAYER_DYING)
	{
		...
		Sanguosha->playAudioEffect(_m_roomSkin->getPlayerAudioEffectPath("sos", player->getGeneral()->isMale()));
	}
	else if (eventType == S_GAME_EVENT_SKILL_INVOKED)
	{
		...
		Sanguosha->playAudioEffect(_m_roomSkin->getPlayerAudioEffectPath(skillName, isMale, type));
	}
}
