...
bool GameRule::trigger(TriggerEvent event, Room* room, ServerPlayer *player, QVariant &data) const{
    ...
    case CardUsed: {
            if(data.canConvert<CardUseStruct>()){
                ...
                if(card_use.from && card_use.to.length() > 1 && !card_use.card->isTargetAsigned()){
...
void BasaraMode::playerShowed(ServerPlayer *player) const{
    ...
    if(answer == "yes"){
        ...
        QString general_name = Config.EnableHegemony ? names.first() : room->askForGeneral(player,names);
...
bool BasaraMode::trigger(TriggerEvent event, Room* room, ServerPlayer *player, QVariant &data) const{
    ...
    case Death:{
        if(Config.EnableHegemony){
            DamageStar damage = data.value<DamageStar>();
            ServerPlayer *killer = damage ? damage->from : NULL;
            if(killer == NULL || killer->getGeneralName() == "anjiang")
                break;

            if(killer->getKingdom() == damage->to->getKingdom()){
                killer->throwAllEquips();
                killer->throwAllHandCards();
            }
