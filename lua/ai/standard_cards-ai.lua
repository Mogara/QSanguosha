...
function sgs.getDefenseSlash(player, self)
	...
	local need_double_jink = attacker:hasSkills("wushuang")
	...
	if attacker:hasSkill("dahe") and player:hasFlag("dahe")	and getKnownCard(player, attacker, "Jink", true, "he") == 0 and getKnownNum(player) == player:getHandcardNum() then
		defense = 0
	end
	...
	for _, slash in ipairs(slashes) do
		...
		for _, friend in ipairs(self.friends_noself) do
			...
			if not self:hasHeavySlashDamage(self.player, card, friend)
				...
				and (self:findLeijiTarget(friend, 50, self.player) or (friend:hasSkill("jieming") and self.player:hasSkill("nosrende") and (huatuo and self:isFriend(huatuo))))
...
function turnUse_spear(self, inclusive, skill_name)
	...
	if #cards <= self.player:getHp() - 1 and self.player:getHp() <= 4 and not self:hasHeavySlashDamage(self.player)
		and not self.player:hasSkills("kongcheng|lianying|paoxiao|shangshi|noshangshi|zhiji|benghuai") then return end
	if #newcards < 2 then return end
...
sgs.ai_skill_invoke.EightDiagram = function(self, data)
	...
	if self.player:hasSkills("tiandu|leiji|nosleiji|gushou") then
...
function SmartAI:willUseGodSalvation(card)
	...
	return (good - bad > 5 and wounded_friend > 0)  or (wounded_friend == 0 and wounded_enemy == 0 and self.player:hasSkill("jizhi"))
end
	...
	if #targets > 0 then
		...
		for i = 1, #targets, 1 do
			...
			if use.to then
				...
				elseif n1 >= enemySlash and not targets[i]:hasSkill("danlao") then
					use.to:append(targets[i])
				end
...
function SmartAI:getDangerousCard(who)
	...
	if weapon and weapon:isKindOf("Axe") and who:hasSkills("luoyi|jiuchi") then
		return weapon:getEffectiveId()
	end
	...
	if (weapon and weapon:isKindOf("SPMoonSpear") and who:hasSkills("guidao|longdan|guicai|qingguo|kanpo")) then
		return weapon:getEffectiveId()
	end
...
function SmartAI:getValuableCard(who)
	...
	for _,equip in ipairs(equips) do
		...
		if who:hasSkills("wusheng|jijiu|xueji") and equip:isRed() then  return equip:getEffectiveId() end
...
function SmartAI:useCardIndulgence(card, use)
	...
	local getvalue = function(enemy)
		...
		if enemy:hasSkills("houyuan|qixi|qice|guose|duanliang|luoshen|jizhi|jilve|wansha|mingce|sizhan") then value = value + 5 end
...
---------------
	...
	if eightdiagram then
		if not self.player:hasSkills("yizhong|bazhen") and self.player:hasSkills("tiandu|leiji|noszhenlie|gushou|hongyan") and not self:getSameEquip(card) then
			return eightdiagram 
		end
		if NextPlayerisEnemy and NextPlayer:hasSkills("tiandu|leiji|noszhenlie|gushou|hongyan") and not self:getSameEquip(card, NextPlayer) then
			return eightdiagram
		end
	end
...
function SmartAI:useCardKnownBoth(KnownBoth, use)
	...
	for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
		if KnownBoth:targetFilter(targets, player, self.player) and sgs.isAnjiang(player) and not targets:contains(player) then
			use.card = KnownBoth
